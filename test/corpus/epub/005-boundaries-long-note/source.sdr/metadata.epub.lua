-- /home/jajpater/Develop/mygithub/koreader-fixtures/corpus/.work/staged/epub/005-boundaries-long-note/source.sdr/metadata.epub.lua
return {
    ["annotations"] = {
        [1] = {
            ["chapter"] = "Fixture",
            ["color"] = "red",
            ["datetime"] = "2026-08-15 13:08:31",
            ["drawer"] = "lighten",
            ["page"] = "/body/DocFragment/body/p[1]/text().0",
            ["pageno"] = 1,
            ["pos0"] = "/body/DocFragment/body/p[1]/text().0",
            ["pos1"] = "/body/DocFragment/body/p[1]/text().30",
            ["text"] = "Absolute EPUB beginning target",
        },
        [2] = {
            ["chapter"] = "Fixture",
            ["color"] = "green",
            ["datetime"] = "2026-08-15 13:08:31",
            ["datetime_updated"] = "2026-08-15 13:08:31",
            ["drawer"] = "lighten",
            ["note"] = "First EPUB note paragraph.\
\
Second paragraph keeps punctuation: [sic], v. 38, and vergaan?\
Unicode remains exact: café, Ελληνικά, 日本語, 😀.",
            ["page"] = "/body/DocFragment/body/p[2]/text().0",
            ["pageno"] = 1,
            ["pos0"] = "/body/DocFragment/body/p[2]/text().0",
            ["pos1"] = "/body/DocFragment/body/p[2]/text().42",
            ["text"] = "A long EPUB note target sits in the middle",
        },
        [3] = {
            ["chapter"] = "Fixture",
            ["datetime"] = "2026-08-15 13:08:31",
            ["page"] = "/body/DocFragment/body/p[3]/text().0",
            ["pageno"] = 2,
            ["text"] = "in Fixture",
        },
        [4] = {
            ["chapter"] = "Fixture",
            ["color"] = "blue",
            ["datetime"] = "2026-08-15 13:08:31",
            ["drawer"] = "lighten",
            ["page"] = "/body/DocFragment/body/p[4]/text().0",
            ["pageno"] = 2,
            ["pos0"] = "/body/DocFragment/body/p[4]/text().0",
            ["pos1"] = "/body/DocFragment/body/p[4]/text().43",
            ["text"] = "The absolute final EPUB text is highlighted",
        },
    },
    ["config_panel_index"] = 1,
    ["copt_b_page_margin"] = 15,
    ["copt_block_rendering_mode"] = 3,
    ["copt_cjk_width_scaling"] = 100,
    ["copt_embedded_css"] = 1,
    ["copt_embedded_fonts"] = 1,
    ["copt_font_base_weight"] = 0,
    ["copt_font_gamma"] = 15,
    ["copt_font_hinting"] = 2,
    ["copt_font_kerning"] = 3,
    ["copt_font_size"] = 22,
    ["copt_h_page_margins"] = {
        [1] = 10,
        [2] = 10,
    },
    ["copt_line_spacing"] = 100,
    ["copt_nightmode_images"] = 1,
    ["copt_render_dpi"] = 96,
    ["copt_rotation_mode"] = 0,
    ["copt_smooth_scaling"] = 0,
    ["copt_status_line"] = 1,
    ["copt_sync_t_b_page_margins"] = 0,
    ["copt_t_page_margin"] = 15,
    ["copt_view_mode"] = 0,
    ["copt_visible_pages"] = 1,
    ["copt_word_expansion"] = 0,
    ["copt_word_spacing"] = {
        [1] = 95,
        [2] = 75,
    },
    ["cre_dom_version"] = 20240114,
    ["css"] = "./data/epub.css",
    ["doc_pages"] = 2,
    ["doc_path"] = "/home/jajpater/Develop/mygithub/koreader-fixtures/corpus/.work/staged/epub/005-boundaries-long-note/source.epub",
    ["doc_props"] = {
        ["identifiers"] = "urn:uuid:f55ef79a-c041-4a70-a94e-e0b000000002",
        ["language"] = "en",
        ["title"] = "KOReader EPUB corpus fixture",
    },
    ["floating_punctuation"] = 0,
    ["font_face"] = "Noto Serif",
    ["font_family_fonts"] = {},
    ["handmade_flows_edit_enabled"] = true,
    ["handmade_flows_enabled"] = false,
    ["handmade_toc_edit_enabled"] = true,
    ["handmade_toc_enabled"] = false,
    ["hide_nonlinear_flows"] = false,
    ["highlight_color"] = "gray",
    ["highlight_drawer"] = "lighten",
    ["hyph_force_algorithmic"] = false,
    ["hyph_soft_hyphens_only"] = false,
    ["hyph_trust_soft_hyphens"] = false,
    ["hyphenation"] = true,
    ["inverse_reading_order"] = false,
    ["invert_ui_layout"] = false,
    ["last_xpointer"] = "/body/DocFragment/body/p[3]/text().0",
    ["page_overlap_style"] = "dim",
    ["partial_md5_checksum"] = "58bcafe8469ffc6056bbf18906dbe4dd",
    ["partial_rerendering"] = true,
    ["percent_finished"] = 1,
    ["readermenu_tab_index"] = 1,
    ["show_overlap_enable"] = false,
    ["stats"] = {
        ["authors"] = "N/A",
        ["highlights"] = 2,
        ["language"] = "en",
        ["notes"] = 1,
        ["pages"] = 2,
        ["performance_in_pages"] = {},
        ["series"] = "N/A",
        ["title"] = "KOReader EPUB corpus fixture",
    },
    ["summary"] = {
        ["modified"] = "2026-08-15",
        ["status"] = "reading",
    },
    ["text_lang"] = "en-US",
    ["text_lang_embedded_langs"] = true,
    ["toc_ticks_ignored_levels"] = {},
}
